...
contract('Hodler vault', function(accounts) {
  ...
  describe('Purchase LP', async () => {
    it('should be possible to purchaseLP', async () => {
      ...
      await osmToken.transfer(USER, tokensAmount);

      await hodlerVault.sendTransaction({value: bn('11').mul(baseUnit)})

      await osmToken.approve(hodlerVault.address, tokensAmount, {from: USER});
      ...
      expectEvent(result, 'LPQueued', {
        hodler: USER,
        osmTokens: tokensAmount.toString()
      });
    });
    ...
    it('should NOT be possible to send 0 tokens on HV via purchaseLP()', async () => {
      ...
      await expectRevert(
        ...
        'HodlerVault: OSM required to mint LP'
      );
    });

    it('should NOT be possible to purchaseLP if user does not have enough OSM tokens', async () => {
      const liquidityTokensAmount = bn('10000').mul(baseUnit); // 10.000 tokens
      const liquidityEtherAmount = bn('10').mul(baseUnit); // 10 ETH
      ...
      await expectRevert(
        ...
        'HodlerVault: Not enough OSM tokens'
      );
    });

    it('should NOT be possible to purchaseLP if user did not do approve to spend OSM on hodlerVault', async () => {
      const liquidityTokensAmount = bn('10000').mul(baseUnit); // 10.000 tokens
      const liquidityEtherAmount = bn('10').mul(baseUnit); // 10 ETH
      ...
      await expectRevert(
        hodlerVault.purchaseLP(tokensAmount, {from: USER}),
        'HodlerVault: Not enough OSM tokens allowance'
      );
    });
  ...
  describe('Claim LP', async () => {
    ...
    it('should NOT be possible to claim LP if already unlocked (2 diff users locked LPs)', async () => {
      ...
      await osmToken.transfer(USER, tokensAmount);
      await osmToken.transfer(USER_2, tokensAmount);

      await hodlerVault.sendTransaction({value: bn('11').mul(baseUnit)})

      await osmToken.approve(hodlerVault.address, tokensAmount, {from: USER});
      await osmToken.approve(hodlerVault.address, tokensAmount, {from: USER_2});
    ...
    it('should be possible to claim LP if already unlocked and user has multiple locks', async () => {
      ...
      await osmToken.transfer(USER, tokensAmount.mul(bn('10')));

      await hodlerVault.sendTransaction({value: bn('11').mul(baseUnit)})

      await osmToken.approve(hodlerVault.address, tokensAmount.mul(bn('10')), {from: USER});
  ...
  describe('Admin functions', async () => {
    it('should be possible to seed more than 1 time for owner', async () => {
      ...
      assert.equal(config.osmToken, fakeAddress);
